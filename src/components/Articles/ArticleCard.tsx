...
export default function ArticleCard({ article }: ArticleCarouselProps) { 
    return (
        <div className="min-w-[450px] p-2">
            <div className="top-4 left-4 p-1 text-gray-500 font-light text-sm mb-2">{article.date}</div> {/* Date */}
            <div className="rounded overflow-hidden">
                <div
                    className={`relative ${article.bgColor} rounded`}
                    style={{ height: '20rem'}}
                >
                {/* Gradient overlay */}
                ...
                {/* <div className=" max-w-[450px] absolute text-black font-medium text-xl mt-4">{article.title}</div> Date */}
